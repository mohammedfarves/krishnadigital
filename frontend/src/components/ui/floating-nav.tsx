...
const FloatingNav = () => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 px-3 pb-3 pt-1 pointer-events-none">
      <nav
        ref={containerRef}
        className="relative flex items-center justify-around bg-card/95 backdrop-blur-md border border-border rounded-2xl shadow-lg py-2 pointer-events-auto"
      >
            ...
            return (
              <SearchModal key={item.id} data={searchData}>
                <button
                  ...
                  className="relative flex flex-col items-center justify-center flex-1 px-2 py-2 text-sm font-medium text-muted-foreground"
                >
          ...
          return (
            ...
            >
              <div className="relative">
                <Icon className="w-5 h-5" />
                {item.badge && (
                  <span className="absolute -top-1.5 -right-1.5 bg-accent text-primary text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                    {item.badge}
                  </span>
                )}
              </div>
        ...
        {active >= 0 && (
          <motion.div
            className="absolute top-0 h-0.5 bg-accent rounded-full"
            initial={false}
            animate={{
